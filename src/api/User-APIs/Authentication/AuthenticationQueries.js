...
export const AuthenticationQueries = {
  selectUserDetailByEmail(email) {
    return pool.query("SELECT id, email, role, password FROM users WHERE email = $1", [email]);
  },
  selectPhoneNumber(phoneNumber) {
    return pool.query("SELECT phone_number FROM users WHERE phone_number = $1", [phoneNumber]);
  },
