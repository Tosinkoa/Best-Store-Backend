...
export const CartQueries = {
  ...
  selectAllCartProductByUserID(userId) {
    return pool.query(
      "SELECT c.product_count, p.name, p.price, p_img.images FROM carts c LEFT JOIN products p ON c.product_id = p.id LEFT JOIN product_images p_img ON p.id = p_img.product_id WHERE user_id = $1",
      [userId]
    );
  },
};
